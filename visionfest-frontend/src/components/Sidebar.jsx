...
export default function Sidebar({ isOpen, onClose }) {
  ...
  return (
    <>
      {/* Overlay */}
      <div
        className={`fixed inset-0 bg-black bg-opacity-50 z-30 md:hidden transition-opacity duration-300 ${isOpen ? 'block' : 'hidden'}`}
        onClick={onClose}
      />

      <aside
        className={`
          fixed top-0 left-0 z-40 h-full w-64 bg-silver text-black p-4 shadow-md
